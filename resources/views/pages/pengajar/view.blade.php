					...
					<ol class="breadcrumbs mb-0">
                        ...
                        @else
                            @foreach ($breadcrumb as $b)
                                <li class="breadcrumb-item {{ $b['css'] }}">
                                    @empty($b['link']) {{ $b['name'] }} @else <a href="{{ $b['link'] }}">{{ $b['name'] }}</a> @endempty
                                </li>
                            @endforeach
                        @endempty
					</ol>
                ...
                <div class="row">
                    <div class="col s12 m6 l4">
                        <div class="card-panel" id="profile-card">
                            ...
                            <div class="row" style="margin-top: 0.8rem">
                                ...
                                <table class="striped">
                                    <tbody>
                                      ...
                                      <tr>
                                        <td>Alamat:</td>
                                        <td class="">{{ $profile->address ?? '-' }} <br>
                                            {{ implode(" ", array($profile->city, $profile->district, $profile->village)) }}
                                        </td>
                                      </tr>

                                      {{-- <tr>
                                        <td>TTL</td>
                                        <td class="">{{ $profile->birth_place }} {{ !is_null($profile->birth_date) ? date(', d/m/Y', strtotime($profile->birth_date)) : '' }}</td>
                                      </tr> --}}
                                      ...
                                      <tr>
                                        <td>Pendidikan</td>
                                        <td class="">{{ $profile->educational_background }} - {{ $profile->educational_field }}</td>
                                      </tr>

                                      <tr>
                                        <td>Profesi</td>
                                        <td class="">{{ $profile->occupation ?? '-' }}</td>
                                      </tr>
                            ...
                            <button type="button" onclick="history.back()" class="btn default">Kembali</button>
                            <a href="/pengajar/edit/{{ $profile->id }}" class="btn green">Edit</a>
                            </div>
                        </div>
                    </div>
