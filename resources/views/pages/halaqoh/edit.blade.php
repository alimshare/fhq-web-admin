...
<nav class="mb-6">
    <ol class="flex items-center gap-x-2 text-sm text-gray-500">
        ...
        <li class="text-gray-900 font-medium">Edit Halaqoh</li>
    </ol>
</nav>

<div class="max-w-lg mx-auto">
    <div class="card">
        <h2 class="text-lg font-semibold text-gray-900 mb-6">Edit Halaqoh</h2>

        <form action="{{ route('halaqoh.editPost', ['halaqohReference'=>$halaqoh->halaqoh_reference]) }}" method="POST" autocomplete="off">
            @csrf
                ...
                <div>
                    ...
                    <select name="program" id="program" class="input-field">
                        ...
                        @foreach($program as $p)
                            <option value="{{ $p->id }}" {{ (!empty($halaqoh->program_id) && $halaqoh->program_id == $p->id) ? 'selected' : '' }}>{{ $p->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="pengajar" id="pengajar" class="input-field">
                        ...
                        @foreach($pengajar as $p)
                            <option value="{{ $p->id }}" {{ (!empty($halaqoh->pengajar_id) && $halaqoh->pengajar_id == $p->id) ? 'selected' : '' }}>{{ $p->name }}</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="flex items-center justify-end gap-3 mt-10 pt-6 border-t border-gray-100">
                <button type="reset" class="btn-secondary" onclick="window.history.back()">Batal</button>
