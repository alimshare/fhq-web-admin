...
class HalaqohController extends Controller
{
    ...
    public function viewRaport(Request $request, $peserta_id = null)
    {
        ...
        if (in_array($peserta->program_name, ["TAKHASSUS"]) || in_array($peserta->program_id, explode(",", env('TAKHOSSUS_IDS', '')))) {
    ...
    function pindah($halaqohId = null, $pesertaId = null)
    {
        ...
        $semesterActive = Semester::getActive();
        $this->data['halaqoh'] = ViewHalaqoh::where('semester_id', $semesterActive->id)->orderBy('pengajar_name')->get();

        if (!empty($halaqohId)) {
            $this->data['peserta'] = ViewPeserta::where('halaqoh_id', $halaqohId)->get();
    ...
    {
        $halaqohAwal = ViewHalaqoh::where('halaqoh_id', $request->old_halaqoh)->first();
        if (!$halaqohAwal) {
            return redirect('/halaqoh/pindah')->with('alert', ['message'=>"Halaqoh Awal tidak valid", 'type'=>'warning']);
        }

        $halaqohTujuan = ViewHalaqoh::where('halaqoh_id', $request->new_halaqoh)->first();
        if (!$halaqohTujuan) {
    ...
    public function manage_v2(Request $request)
    {
        ...
        $program = Program::select('id','name')->orderBy('sequence','asc')->get();
        $queryHalaqoh = ViewHalaqoh::select('halaqoh_id','program_id', 'pengajar_name', 'day','halaqoh_reference','jenis_kbm','program_name','halaqoh_gender')
            ->where('semester_id', $semester)
            ->orderBy('gender')
            ->orderBy('jenis_kbm')
            ->withCount(['peserta']);
        if (!empty($request->kbm)) $queryHalaqoh->where('jenis_kbm', $request->kbm);
        if (!empty($request->program)) $queryHalaqoh->where('program_id', $request->program);
        if (!empty($request->gender)) $queryHalaqoh->where('halaqoh_gender', $request->gender);
        $halaqohs = $queryHalaqoh->get();

        $days = explode(",", strtoupper(env('AVAILABLE_DAYS', 'SABTU,AHAD')) );
        $semesterList = Semester::orderByDesc('id')->get();
        $programList = Program::select('id','name')->orderBy('sequence','asc')->get();
        ...
        return view('pages.halaqoh.manage-v2', compact('program', 'data', 'days', 'semesterList', 'programList'));
    }
